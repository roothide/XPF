...
uint64_t xpf_find_ppl_dispatch_section(void) {
    ...
    PFPatternMetric *metric = pfmetric_pattern_init(pplCallerInst, pplCallerMask, sizeof(pplCallerInst),
                                                    sizeof(uint32_t));
...
uint64_t xpf_find_ppl_enter(void) {
    ...
    arm64_dec_b_l(pfsec_read32(gXPF.kernelTextSection, ppl_dispatch_section + 4), ppl_dispatch_section + 4, &ppl_enter,
                  NULL);
    return ppl_enter;
}
...
uint64_t xpf_find_ppl_bootstrap_dispatch(void) {
    ...
    if (cbzPPLDispatchAddr) {
        ...
        arm64_dec_cb_n_z(pfsec_read32(gXPF.kernelTextSection, cbzPPLDispatchAddr), cbzPPLDispatchAddr, NULL, NULL,
                         &ppl_bootstrap_dispatch);
        return ppl_bootstrap_dispatch;
    } else {
        uint32_t bCondAnyInst = 0, bCondAnyMask = 0;
        arm64_gen_b_c_cond(OPT_BOOL(false), OPT_UINT64_NONE, OPT_UINT64_NONE, ARM64_COND_ANY, &bCondAnyInst,
                           &bCondAnyMask);
        uint64_t bcondPPLDispatchAddr = pfsec_find_next_inst(gXPF.kernelTextSection, ppl_enter, 30, bCondAnyInst,
                                                             bCondAnyMask);
        if (bcondPPLDispatchAddr) {
            uint64_t ppl_bootstrap_dispatch = 0;
            arm64_dec_b_c_cond(pfsec_read32(gXPF.kernelTextSection, bcondPPLDispatchAddr), bcondPPLDispatchAddr,
                               &ppl_bootstrap_dispatch, NULL, NULL);
            return ppl_bootstrap_dispatch;
        }
    }
    return 0;
...
uint64_t xpf_find_pmap_image4_trust_caches(void) {
    uint64_t pmap_lookup_in_loaded_trust_caches_internal = xpf_item_resolve(
            "kernelSymbol.pmap_lookup_in_loaded_trust_caches_internal");
    ...
    uint64_t ldrLitAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection, pmap_lookup_in_loaded_trust_caches_internal,
                                               20, ldrLitAny, ldrLitAnyMask);
    if (ldrLitAddr) {
...
uint64_t xpf_find_pmap_query_trust_cache_safe(void) {
    ...
    uint64_t blAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection, pmap_lookup_in_loaded_trust_caches_internal, 30,
                                           blAny, blAnyMask);
...
uint64_t xpf_find_ppl_trust_cache_rt(void) {
    ...
    uint64_t pacMovAddr = pfsec_find_next_inst(gXPF.kernelPPLTextSection, pmap_query_trust_cache_safe, 50, pacMovInst,
                                               pacMovMask);
    ...
    uint64_t ppl_trust_cache_rt_refAddr = pfsec_find_prev_inst(gXPF.kernelPPLTextSection, pacMovAddr, 10, addX0AnyInst,
                                                               addX0AnyMask);
...
uint64_t xpf_find_tte_get_ptd(void) {
    ...
    if (!tte_get_ptd_stringAddr) {
        ...
        stringMetric = pfmetric_string_init(
...
void xpf_ppl_init(void) {
    if (gXPF.kernelIsArm64e) {
        ...
        xpf_item_register("kernelSymbol.pmap_lookup_in_loaded_trust_caches_internal", xpf_find_ppl_routine,
                          (void *) (uint32_t) 41);
        ...
        xpf_item_register("kernelSymbol.vm_first_phys", xpf_find_pmap_pin_kernel_pages_reference,
                          (void *) (uint32_t) 0);
        xpf_item_register("kernelSymbol.vm_last_phys", xpf_find_pmap_pin_kernel_pages_reference, (void *) (uint32_t) 1);
        xpf_item_register("kernelSymbol.pp_attr_table", xpf_find_pmap_pin_kernel_pages_reference,
                          (void *) (uint32_t) 2);
