...
uint64_t xpf_find_hw_lck_ticket_reserve_orig_allow_invalid_signed(void) {
    ...
    arm64_gen_str_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_X(10), ARM64_REG_X(16), OPT_UINT64_NONE, &strX10X16Any,
                      &strX10X16AnyMask);
...
uint64_t xpf_find_hw_lck_ticket_reserve_orig_allow_invalid(void) {
    ...
    return pfsec_find_prev_inst(gXPF.kernelTextSection, hw_lck_ticket_reserve_orig_allow_invalid_signed, 40, adrAny,
                                adrAnyMask);
}
...
uint64_t xpf_find_str_x0_x19_ldr_x20_gadget(void) {
    ...
    arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_ANY, ARM64_REG_X(20), OPT_UINT64_NONE, &ldrAnyX20Any,
                      &ldrAnyX20AnyMask);
...
uint64_t xpf_find_thread_recover(void) {
    uint32_t strInst = pfsec_read32(gXPF.kernelTextSection,
                                    xpf_item_resolve("kernelGadget.hw_lck_ticket_reserve_orig_allow_invalid_signed"));
    uint64_t imm = 0;
    ...
    if (!machine_kstackptr) {
        ...
        pfmetric_run(gXPF.kernelTextSection, patternMetric2, ^(uint64_t vmaddr, bool *stop) {
            if (pfsec_find_next_inst(gXPF.kernelTextSection, vmaddr, 0x10, movk_w12_0x4Inst, movk_w12_0x4Mask)) {
                arm64_dec_ldr_imm(pfsec_read32(gXPF.kernelTextSection, vmaddr + 4), NULL, NULL, &machine_kstackptr,
                                  NULL, NULL);
                *stop = true;
            }
        });
...
uint64_t xpf_find_thread_machine_CpuDatap(void) {
    ...
    arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_X(11), ARM64_REG_X(10), OPT_UINT64_NONE, &ldrX11X10AnyInst,
                      &ldrX11X10AnyMask);
...
void xpf_bad_recovery_init(void) {
    if (gXPF.kernelIsArm64e && xpf_bad_recovery_supported()) {
        xpf_item_register("kernelSymbol.hw_lck_ticket_reserve_orig_allow_invalid",
                          xpf_find_hw_lck_ticket_reserve_orig_allow_invalid, NULL);
        xpf_item_register("kernelGadget.hw_lck_ticket_reserve_orig_allow_invalid_signed",
                          xpf_find_hw_lck_ticket_reserve_orig_allow_invalid_signed, NULL);
        ...
        xpf_item_register("kernelGadget.exception_return_after_check_no_restore",
                          xpf_find_exception_return_after_check_no_restore, NULL);
