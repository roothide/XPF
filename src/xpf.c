...
int xpf_start_with_kernel_path(const char *kernelPath) {
    ...
    if (gXPF.kernelIsFileset) {
        ...
        gXPF.kernelAMFITextSection = pfsec_init_from_macho(gXPF.kernel, "com.apple.driver.AppleMobileFileIntegrity",
                                                           "__TEXT_EXEC", "__text");
        gXPF.kernelAMFIStringSection = pfsec_init_from_macho(gXPF.kernel, "com.apple.driver.AppleMobileFileIntegrity",
                                                             "__TEXT", "__cstring");
        gXPF.kernelSandboxTextSection = pfsec_init_from_macho(gXPF.kernel, "com.apple.security.sandbox", "__TEXT_EXEC",
                                                              "__text");
        gXPF.kernelSandboxStringSection = pfsec_init_from_macho(gXPF.kernel, "com.apple.security.sandbox", "__TEXT",
                                                                "__cstring");
        gXPF.kernelInfoPlistSection = pfsec_init_from_macho(gXPF.kernel, "com.apple.security.AppleImage4", "__TEXT",
                                                            "__info_plist");
    } else {
    ...
    gXPF.kernelEntry = 0;
    macho_enumerate_load_commands(gXPF.kernel,
                                      ...
                                      if (loadCommand.cmd == LC_SEGMENT_64) {
                                          ...
                                          while (cmdData < cmdDataEnd) {
                                              ...
                                              if (flavor == ARM_THREAD_STATE64) {
                                                  arm_thread_state64_t *threadState = (arm_thread_state64_t *) (
                                                          cmdData + 8);
#ifdef __arm64e__
    ...
    PFPatternMetric *versionMetric = pfmetric_pattern_init((void *) versionSearchString, NULL,
                                                           strlen(versionSearchString), sizeof(uint8_t));
    ...
    if (gXPF.kernelInfoPlistSection) {
        ...
        PFPatternMetric *infoPlistMetric = pfmetric_pattern_init((void *) infoPlistString, NULL,
                                                                 strlen(infoPlistString), sizeof(uint8_t));
    ...
    if (!gXPF.kernelVersionString) {
        ...
        sscanf(gXPF.kernelVersionString, "Darwin Kernel Version %[^:]: %*[^;]; root:xnu-%[^/]/%s", darwinVersion,
               xnuBuild, xnuPlatform);
