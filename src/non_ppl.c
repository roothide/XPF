...
uint64_t xpf_find_non_ppl_trust_cache_rt(void) {
    ...
    return xpfsec_read_ptr(gXPF.kernelDataConstSection,
                           pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, adrpAddr + 4));
}
...
void xpf_non_ppl_init(void) {
    if (!gXPF.kernelIsArm64e) {
        ...
        xpf_item_register("kernelSymbol.vm_first_phys", xpf_find_non_ppl_pmap_tt_deallocate_reference,
                          (void *) (uint32_t) 1);
        xpf_item_register("kernelSymbol.pv_head_table", xpf_find_non_ppl_pmap_tt_deallocate_reference,
                          (void *) (uint32_t) 2);
