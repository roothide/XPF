...
uint64_t xpf_find_pointer_mask_symbol(uint32_t n) {
    ...
    arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_Q(0), ARM64_REG_ANY, OPT_UINT64_NONE, &ldrQ0AnyInst,
                      &ldrQ0AnyMask);
...
uint64_t xpf_find_pointer_mask(void) {
    ...
    if (pointer_mask != 0xffffff8000000000 && pointer_mask != 0xffff800000000000 &&
        pointer_mask != 0xffffffc000000000) {
...
uint64_t xpf_find_PT_INDEX_MAX(void) {
    ...
    PFStringMetric *stringMetric = pfmetric_string_init(
            "%s: out of PTD entries and for some reason didn't allocate more %d %p");
...
uint64_t xpf_find_fatal_error_fmt(void) {
    ...
    PFStringMetric *amfiErrorMetric = pfmetric_string_init(
            "AMFI: %s: Failed to allocate memory for fatal error message, cannot produce a crash reason.\n");
...
uint64_t xpf_find_kfree_data_external(void) {
    ...
    if (fatal_error_fmt_end) {
        ...
        arm64_gen_mov_imm('z', ARM64_REG_W(1), OPT_UINT64(0x400), OPT_UINT64(0), &movW1_0x400_Inst,
                          &movW1_0x400_InstMask);
        uint64_t kfree_data_external_pre = pfsec_find_prev_inst(sec, fatal_error_fmt_end, 25, movW1_0x400_Inst,
                                                                movW1_0x400_InstMask);

        uint64_t kfree_data_external = 0;
        if (arm64_dec_b_l(pfsec_read32(sec, kfree_data_external_pre + 4), kfree_data_external_pre + 4,
                          &kfree_data_external, NULL) == 0) {
...
uint64_t xpf_find_task_collect_crash_info(void) {
    ...
    PFXrefMetric *task_crashinfo_release_refXrefMetric = pfmetric_xref_init(task_crashinfo_release_ref,
                                                                            XREF_TYPE_MASK_CALL);
...
uint64_t xpf_find_task_itk_space(void) {
    ...
    pfmetric_run(gXPF.kernelTextSection, task_collect_crash_infoXrefMetric, ^(uint64_t vmaddr, bool *stop) {
        ...
        arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_ANY, ARM64_REG_ANY, OPT_UINT64_NONE, &ldrAnyInst,
                          &ldrAnyMask);
...
uint64_t xpf_find_vm_reference(uint32_t idx) {
    ...
    pfmetric_run(gXPF.kernelTextSection, patternMetric, ^(uint64_t vmaddr, bool *stop) {
        uint32_t ldrAny = 0, ldrAnyMask = 0;
        arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_ANY, ARM64_REG_ANY, OPT_UINT64_NONE, &ldrAny,
                          &ldrAnyMask);
...
uint64_t xpf_find_vm_map_pmap(void) {
    ...
    uint32_t inst[2] = {0};
    uint32_t mask[2] = {0};
    arm64_gen_ldr_imm(0, gXPF.kernelIsArm64e ? LDR_STR_TYPE_PRE_INDEX : LDR_STR_TYPE_UNSIGNED, ARM64_REG_ANY,
                      ARM64_REG_ANY, OPT_UINT64_NONE, &inst[0], &mask[0]);
    ...
    pfmetric_run_in_range(gXPF.kernelTextSection, xrefAddr, xrefAddr - (100 * sizeof(uint32_t)), patternMetric,
                          ^(uint64_t vmaddr, bool *stop) {
                              if (arm64_dec_adr_p(pfsec_read32(gXPF.kernelTextSection, vmaddr - 4), vmaddr - 4, NULL,
                                                  NULL, NULL) != 0) {
                                  arm64_dec_ldr_imm(pfsec_read32(gXPF.kernelTextSection, vmaddr), NULL, NULL,
                                                    &vm_map_pmap, NULL, NULL);
                                  *stop = true;
                              }
                          });
    pfmetric_free(patternMetric);
...
uint64_t xpf_find_proc_struct_size(void) {
    if (strcmp(gXPF.darwinVersion, "22.0.0") >= 0) {
        ...
        arm64_gen_ldr_imm(0, LDR_STR_TYPE_UNSIGNED, ARM64_REG_ANY, ARM64_REG_ANY, OPT_UINT64_NONE, &ldrAnyInst,
                          &ldrAnyMask);
        uint64_t ldrAddr = pfsec_find_prev_inst(gXPF.kernelTextSection, procTaskStringXref, 0x20, ldrAnyInst,
                                                ldrAnyMask);

        uint64_t proc_struct_sizeAddr = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection,
                                                                                            ldrAddr);
        ...
        PFPatternMetric *patternMetric = pfmetric_pattern_init(&procStringAddr, &mask, sizeof(procStringAddr),
                                                               sizeof(uint64_t));
...
uint64_t xpf_find_perfmon_devices(void) {
    ...
    uint64_t movAddr = pfsec_find_next_inst(gXPF.kernelTextSection, perfmon_dev_open, 0, movWAny_0x28Inst,
                                            movWAny_0x28Mask);
...
uint64_t xpf_find_cdevsw(void) {
    ...
    */
    ...
    for (int i = 0; i < skip; i++) {
        bl_spec_kqfilterAddr = pfsec_find_next_inst(gXPF.kernelTextSection, bl_spec_kqfilterAddr + 4, 100, blAny,
                                                    blAnyMask);
    }

    uint64_t spec_kqfilter = 0;
    arm64_dec_b_l(pfsec_read32(gXPF.kernelTextSection, bl_spec_kqfilterAddr), bl_spec_kqfilterAddr, &spec_kqfilter,
                  NULL);
...
uint64_t xpf_find_nsysent(void) {
    ...
    uint64_t mov1Addr = pfsec_find_next_inst(gXPF.kernelTextSection, proc_get_syscall_filter_mask_size, 20, movAnyInst,
                                             movAnyMask);
...
uint64_t xpf_find_mach_trap_count(void) {
    ...
    uint32_t movAnyInst = 0, movAnyMask = 0;
    arm64_gen_mov_imm('z', ARM64_REG_ANY, OPT_UINT64_NONE, OPT_UINT64_NONE, &movAnyInst, &movAnyMask);
    uint64_t movAddr = pfsec_find_next_inst(gXPF.kernelTextSection, proc_get_syscall_filter_mask_size, 20, movAnyInst,
                                            movAnyMask);
...
uint64_t xpf_find_mach_kobj_count(void) {
    ...
    uint64_t ldrswAddr = pfsec_find_next_inst(gXPF.kernelTextSection, proc_get_syscall_filter_mask_size, 40, ldrswInst,
                                              ldrswMask);
...
uint64_t xpf_find_developer_mode_enabled(void) {
    ...
    uint32_t adrpAnyInst = 0, adrpAnyMask = 0;
    arm64_gen_adr_p(OPT_BOOL(true), OPT_UINT64_NONE, OPT_UINT64_NONE, ARM64_REG_ANY, &adrpAnyInst, &adrpAnyMask);
    uint64_t branchTarget = pfsec_find_prev_inst(gXPF.kernelTextSection, stringXrefAddr - 4, 20, adrpAnyInst,
                                                 adrpAnyMask);
...
void xpf_common_init(void) {
    ...
    xpf_item_register("kernelSymbol.proc_get_syscall_filter_mask_size", xpf_find_proc_get_syscall_filter_mask_size,
                      NULL);
